...
import { AlertsPanel } from '@/components/incidents/AlertsPanel';
...
function Metric({ label, value, tone = 'text-ink' }: { label: string; value: string; tone?: string }) {
  return (
    <div className="min-w-0">
      ...
      <p className={`mt-1 truncate font-mono text-[15px] font-medium ${tone}`}>{value}</p>
    </div>
  );
}
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-3 overflow-hidden p-3">
      <header className="flex shrink-0 flex-wrap items-center justify-between gap-3 rounded-panel border border-edge bg-panel px-4 py-3">
        ...
        <div className="grid shrink-0 grid-cols-3 gap-6 sm:grid-cols-6">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-3 xl:grid-cols-[300px_minmax(0,1fr)_320px]">
        <MissionAreasPanel className="min-h-0" onCreate={() => navigate('/missions')} />
        <CommandMap className="min-h-[320px] xl:min-h-0" />
        ...
        <div className="flex min-h-0 flex-col gap-3">
          <div className="shrink-0 rounded-panel border border-edge bg-panel p-3.5">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Metric label="State" value={humanise(telemetry.missionStatus)} />
          ...
          <AlertsPanel limit={4} className="min-h-0 flex-1" />
        </div>
      </div>
