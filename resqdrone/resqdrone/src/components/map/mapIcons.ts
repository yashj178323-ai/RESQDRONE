...
export function droneIcon(heading: number, degraded: boolean, p: MarkerPalette) {
  const stroke = degraded ? p.warn : p.info;
  return icon(
...
export function teamIcon(state: TeamState, p: MarkerPalette) {
  ...
  return icon(
    `<div style="width:28px;height:28px;border-radius:7px;background:${p.surface};border:2px solid ${c};display:grid;place-items:center;box-shadow:0 1px 3px rgba(15,23,42,0.18);">
...
export function baseIcon(p: MarkerPalette) {
  return icon(
    `<div style="width:30px;height:30px;border-radius:7px;background:${p.surface};border:2px solid ${p.info};display:grid;place-items:center;box-shadow:0 1px 3px rgba(15,23,42,0.18);">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="${p.info}" stroke-width="2.2" stroke-linecap="round">
