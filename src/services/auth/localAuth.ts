...
/**
 * Only the identifier and the login time are stored. The operator's display
 * name, role and initials are looked up from the operator record on every read,
 * so changing them takes effect immediately instead of being pinned to whatever
 * a browser happened to save during an earlier session.
 */
  ...
  for (const store of [window.localStorage, window.sessionStorage]) {
    ...
    try {
      ...
      if (
        ...
        // Profile fields come from the operator record, never from storage.
        return { ...DEMO_OPERATOR, operatorId: parsed.operatorId, loginAt: parsed.loginAt };
      }
      // Wrong shape or an older schema: discard rather than show stale identity.
