...
export function systemReducer(state: SystemSlice, action: AppAction): SystemSlice {
  switch (action.type) {
    ...
    case 'system/audit':
      return { ...state, audit: [action.event, ...state.audit].slice(0, 250) };

    case 'system/readAlerts':
      return { ...state, alerts: state.alerts.map((a) => ({ ...a, read: true })) };

    case 'system/setConnection': {
      const next = { ...state.connection, ...action.patch };
      if ('gps' in action.patch) {
        next.navMode = action.patch.gps === 'LOCKED' ? 'GPS' : 'GPS_DENIED';
        next.opticalFlow = action.patch.gps === 'LOCKED' ? 'READY' : 'ACTIVE';
        next.rangeSensor = 'ACTIVE';
      }
