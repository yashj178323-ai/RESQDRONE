...
function ZoneDetail({ area }: { area: SearchArea }) {
  return (
    <div className="rounded-control border border-ok/45 bg-ok/[0.06] p-3.5">
      ...
      {/*
        Flight time renders as two segments ("14m 11s"); equal thirds were too
        narrow so it collided with the neighbouring value. Auto-sized columns
        give the time value the room it needs.
      */}
      <dl className="mt-3 grid grid-cols-[auto_auto_minmax(0,1fr)] gap-x-4 gap-y-2 border-t border-edge pt-2.5">
        <div className="min-w-0">
        ...
        <div className="min-w-0">
          ...
          <dd className="mt-1 whitespace-nowrap font-mono text-[14px] text-ink">
...
export function MissionAreasPanel({
  ...
  return (
    ...
    >
      ...
      {/*
        Only the zone list scrolls. Header and tabs remain fixed.
        The list is always a scroll container; when content fits, there is
        simply nothing to scroll.
      */}
