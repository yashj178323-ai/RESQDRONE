...
export function VerificationCard({
  ...
  // Critical detections show their justification immediately; lower priorities
  // stay collapsed so the panel does not grow without cause.
  const [showAnalysis, setShowAnalysis] = useState(detection.priority === 'P1');
      ...
      <div className="space-y-3 p-3.5">
        ...
        {survivor ? (
          ...
          <>
            ...
            {/* Navigation, deliberately separated and visually quieter. */}
            <button
              type="button"
              onClick={() => focusOn(detection.latitude, detection.longitude, detailZoom)}
              className="flex w-full items-center justify-center gap-1.5 border-t border-edge pt-2.5 text-[12px] text-muted transition-colors ease-ui hover:text-brand"
            >
