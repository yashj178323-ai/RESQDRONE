...
export function ThermalViewport({ className = '' }: { className?: string }) {
  ...
  const draw = useCallback(() => {
    ...
    /*
     * The AMG8833 is a physical 8x8 array, so the frame is drawn as 64 discrete
     * cells. Bilinear smoothing made the simulated output look like a CSS
     * gradient and implied resolution the sensor does not have.
     */
    ...
    for (let y = 0; y < size; y += 1) {
      for (let x = 0; x < size; x += 1) {
        const [r, g, b] = samplePalette(palette, (grid[y * size + x] - minC) / span);
        ctx.fillStyle = `rgb(${r},${g},${b})`;
        // Ceil the extents so neighbouring cells never leave seam lines.
        ctx.fillRect(
          Math.floor(x * cellW),
          Math.floor(y * cellH),
          Math.ceil(cellW) + 1,
  ...
  return (
    ...
    >
      ...
      <footer className="flex shrink-0 items-center gap-3 overflow-hidden border-t border-edge px-3 py-1.5 font-mono text-[10px] text-dim">
        ...
        <span title="Alarm threshold: the temperature at which a signature is raised">
          ALARM {thresholdC.toFixed(1)}°C
        </span>
        <span
          className={detected ? 'text-crit' : 'text-ink'}
          title="Hottest cell in the current frame"
        >
          PEAK {maxC.toFixed(1)}°C
        </span>
        <span className={`ml-auto shrink-0 ${detected ? 'text-crit' : 'text-muted'}`}>
          {detected ? 'ABOVE ALARM' : 'BELOW ALARM'}
        </span>
      </footer>
    </section>
  );
